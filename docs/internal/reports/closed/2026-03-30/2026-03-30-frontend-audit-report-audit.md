...
```css
:root {
  --status-bar-bg: color-mix(in srgb, var(--background) 85%, transparent);
}
.dark {
  --status-bar-bg: color-mix(in srgb, var(--card) 85%, transparent);
}
```
...
_Next audit recommended: After FileManagerPage decomposition_
