...
> **Source of truth:** This document describes the **current** tree under `src/` and `src-tauri/`. Prefer code over stale memory-bank notes when they diverge.
...
│   └── tauri*.conf.json
├── docs/                         # Architecture, reports, plans, references
├── memory-bank/                  # Working agent context (may lag code)
...
**Agent rule:** run full gate only after **all** tasks for a request are finished — never mid-task.
...
| Path | Use |
| --- | --- |
| `AGENTS.md` | Agent working rules, verification policy, layout/Rust rules |
| `memory-bank/*` | Living project context (verify against code) |
| `docs/reports/active/*` | Audits and packaging notes |
| `docs/superpowers/plans/*` | Historical implementation plans |
| `.agents/rules/ultracite.md` | FE lint standards (Ultracite) |

---

*Generated from live codebase exploration (frontend shell, desktop IPC, Rust commands/domains, and feature flows). Update this file when boundaries, command surface, or shell layout change.*
