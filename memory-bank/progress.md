...
- Payload Dumper: 4-tab Precision Hardware Cockpit (`PayloadDumperHeroBanner` with container format detection, uncompressed footprint, and destination reveal, `PayloadOverviewTab` clean overview screen, `PayloadExtractorTab` with integrated local/remote source ingestion and searchable partition grid, and `PayloadHistoryTab` with output files explorer)
...
- **Universal Firmware Hub Backend (`src-tauri/src/firmware/`)**: Pluggable `FirmwareProvider` trait with 5 live OEM providers: `GooglePixelScraper` (Factory & OTA with ToS bypass), `NothingProvider` (Nothing OS & CMF Docusaurus HTML scraper), `XiaomiProvider` (multi-feed YAML scraper with 4,600+ builds across 340+ models), `OnePlusProvider` (spike0en/oneplus_archive releases scraper with direct OTA CDN linking and 50+ models), and `SamsungProvider` (live FOTA version.xml querying with 70+ models across Galaxy S/Z/Note/A/Tab series), backed by two-tier caching (RAM `RwLock` + 24h disk JSON TTL at `<cache_dir>/firmware/`).
- **CrAU Dynamic Manifest & Header Engine (`src-tauri/src/payload/crau/` & `src-tauri/src/payload/remote/mod.rs`)**: `read_remote_crau_header` dynamically reading the exact Protobuf `DeltaArchiveManifest` length from the 32-byte header prefix, resolving 1MB truncations on multi-MB modern OTA manifests (e.g. Nothing OS 2.06MB manifest).
- **SSRF-Validated Range Request Redirect Engine (`src-tauri/src/payload/remote/http.rs`)**: `validated_redirect_policy` and `validated_blocking_redirect_policy` enforcing per-hop SSRF validation while following up to 5 `301`/`302`/`307`/`308` redirects with preserved `Range: bytes={start}-{end}` headers, resolving `302 Found` errors from Google/Nothing OTA CDNs (`android.googleapis.com` $\rightarrow$ `*.gvt1.com`).
- **Universal Frontend Hub & Precision Extractor Workspace (`src/features/payload-dumper/`)**: Reorganized tab order (Overview $\rightarrow$ Extractor $\rightarrow$ Firmware Hub $\rightarrow$ History), 1-click **Remote Stream Extract** from Firmware Hub straight into active streaming, persistent Local/Remote source tabs, Recent Links history bar, brand-specific segmented controls (`Recovery ROM (Remote Stream)` vs `Fastboot ROM (TGZ)` for Xiaomi), and high-precision `RemoteLoadProgressCard`.
- **Device Name Normalization**: Rust scraper `extract_device_name` and frontend `formatCleanDeviceName` stripping `"codename" for ` HTML prefixes, parentheses, and quotes across all device entries.
- **Dedicated Payload Module Guide & Agent Routing (`src-tauri/src/payload/AGENTS.md`)**: Complete 350+ line payload domain guide with zero-copy memory model, dynamic partition algorithms, delta engine specs, sparse IOCTL thresholds, and safety invariants, referenced directly from root and backend module guides.
- **Verification & Quality**: Tested real 3+ GB ROM zip extraction on `EvolutionX-15.0-20260415-marble-10.16-Unofficial.zip`, 46/46 Vitest files (269/269 tests) passing, 0 TypeScript errors, 0 Biome/Ultracite lint errors across 491 files, and 0 Cargo compiler warnings.
| Item | Note |
| --- | --- |
