...
- Payload Dumper: 4-tab Precision Hardware Cockpit (`PayloadDumperHeroBanner` with container format detection, uncompressed footprint, and destination reveal, `PayloadOverviewTab` clean overview screen, `PayloadExtractorTab` with searchable partition grid and output destination picker, `PayloadSourceTab` with local/remote stream loaders, and `PayloadHistoryTab` with output files explorer)
...
- **Search & Model Filter Controls**: Global search input on the left, searchable **All Models Combobox Dropdown** on the right (`Popover` + `Command` with `align="end"`), listing all models with codenames and release years with instant filtering.
...
- **Verification & Quality**: Tested real 3+ GB ROM zip extraction on `C:\Users\akila\OneDrive\Desktop\AGN-Test\payloaddumper\EvolutionX-15.0-20260415-marble-10.16-Unofficial.zip`, 28/28 payload dumper Vitest tests passing, 0 TypeScript errors, 0 Biome lint errors across 492 files, and 0 Cargo compiler warnings.
| Item | Note |
| --- | --- |
