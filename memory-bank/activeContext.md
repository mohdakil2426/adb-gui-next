...
9. **Universal Frontend Hub & Integrated Extractor Workspace (`src/features/payload-dumper/`)**: Multi-brand selector chips (`All`, `Google Pixel`, `Nothing`, `Xiaomi`, `OnePlus`, `Samsung`), TanStack Query `useFirmwareCatalog`, live loading skeletons, 1-click **Remote Stream Extract** bridge into payload dumper, dynamic partition sub-unpack controls in `PayloadOverviewTab`, integrated local/remote ingestion in `PayloadExtractorTab`, and complete removal of static mock data.
...
13. **Nothing OS & CMF by Nothing Live Scraper (`src-tauri/src/firmware/providers/nothing.rs`)**: Complete live Docusaurus v3 HTML scraper targeting `https://nothingarchive.tech/docs/firmware`, parsing all 12 devices across Nothing Phone and CMF series (`Phone (1)`, `Phone (2)`, `Phone (2a)`, `Phone (2a) Plus`, `Phone (3)`, `Phone (3a)`, `Phone (3a) Lite`, `Phone (4a)`, `Phone (4a) Pro`, `Phone (4b)`, `CMF Phone (1)`, `CMF Phone (2) Pro`), extracting Full OTAs and Incremental OTAs from Google OTA API, Nothing CloudFront, and Archive.org, paired with automated Android version and release date parsing from build IDs (`B` -> Android 16, `V` -> Android 15, `U` -> Android 14, `T` -> Android 13, `S` -> Android 12) and 24-hour two-tier disk caching.
| Area | Decision |
| --- | --- |
