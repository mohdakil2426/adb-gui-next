...
**Payload Dumper interaction polish is in progress/completed for this session (2026-05-11).** The loaded-state payload banner now shows an explicit `Source` line directly under the payload name for both local paths and remote URLs, while keeping the original compact banner structure. The partition search input was moved to sit directly above the partition table, centered within the table surface instead of floating between banner and list state. The bottom action area now uses a wide two-button row (`Reset` + `Extract`, or `Reset` + `Cancel` while extracting) that matches the table width better. `ExtractionStatusCard` was compacted into a denser summary layout, `PartitionRow` now wraps long partition names and appends `.img`, and the redundant theme toggle was removed from the sidebar footer so the header remains the single theme control. Global Sonner toasts now mount at the top-right.
