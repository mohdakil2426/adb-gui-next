...
| Payload Dumper | 4-tab Hardware Cockpit: `PayloadDumperHeroBanner` (format detection, uncompressed footprint, partition counts, folder reveal) + Overview (clean overview screen) + Extractor & Partitions (filter chips, search, `PartitionTableList`, destination picker) + Source & Remote Loader (local dropzone and remote HTTP stream loader with device catalog presets) + Extracted Outputs & History (`PayloadHistoryTab` output explorer) |
