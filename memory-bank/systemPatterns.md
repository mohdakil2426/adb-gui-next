...
- Payload: mmap/streaming; cancel tokens; `TransactionGuard` file-only cleanup; dynamic CrAU manifest reader (`read_remote_crau_header`); `validated_redirect_policy` for SSRF-safe HTTP range request follow (Google/Nothing OTA 302 redirects); ZIP64 CD extras for remote factory ZIP.
- Firmware Hub: Pluggable `FirmwareProvider` architecture (Google Pixel, Nothing OS, Xiaomi/Redmi/POCO, OnePlus, Samsung); concurrent multi-feed scrapers with 24h two-tier disk cache (`<cache_dir>/firmware/<brand>.json`) and offline fallback; base codename normalization and hardware SoC/year/series enrichment.
...
| Area           | Pattern                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Marketplace    | 4-tab Hardware Cockpit: `MarketplaceHeroBanner` (pulsating sync badge, device compatibility indicator, total apps, repository badges) + Overview (`SourceCompositionDonut`, `CategoryDistributionMeter`, `QuickDiscoveryTags`) + Browse & Search (search bar, filter bar, grid/list mode, in-place `AppDetailView`) + Installed & Updates (`AppUpdateRow` with 1-click Update All batch runner) + Sources & Repos (`RepositorySourcesCard`, `GithubAuthCard` PAT rate limit, `CachePreferencesCard`) |
...
| Payload Dumper | 4-tab Hardware Cockpit: `PayloadDumperHeroBanner` (format detection, uncompressed footprint, partition counts, destination reveal) + Overview (clean overview screen with workflow shortcuts and dynamic LpMetadata detection) + Extractor & Partitions (search, category filters, `PartitionTableList`, persistent local/remote source tabs, destination picker, `ActionFooter`) + Firmware Hub (OEM multi-brand catalog with Google Pixel, Nothing & CMF, Xiaomi/Redmi/POCO, OnePlus, and Samsung Galaxy models, 1-click Remote Stream Extract) + Extracted Outputs & History (`PayloadHistoryTab` output explorer)                                                                                                                                                                                                                                                                                    |
| Utilities      | 5-tab Hardware Cockpit: `UtilitiesCockpitHero` (live ADB daemon port 5037 status, host toolchain versions, connected serial/mode, and server restart) + Overview & Quick Actions (`InstantActionsCard` symmetrical 3x2 command grid, `AdbTransportGuideCard` interactive socket architecture guide) + Power & Tweaks (`TargetRebootCard` symmetrical 3x2 reboot grid, `SystemTweaksCard` window animations, dark mode, DPI) + Diagnostics (`ScreenshotStudioCard` frame capture, `LogcatStreamCard` real-time log buffer filter) + Fastboot (`FastbootSlotControlCard` active slot switcher & power, `FastbootGetVarCard` deep `getvar all` variable inspector) + Host Setup (`HostServerControlCard` server controls, `HostGoogleSetupCard` official standalone platform-tools & USB driver installer)                                                                                                  |
