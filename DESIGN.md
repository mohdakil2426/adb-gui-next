...
| **Payload Dumper** | 4-tab Hardware Cockpit: `PayloadDumperHeroBanner` (format detection, uncompressed footprint, partition counts, folder reveal) + tabs:<br>• **Overview & Telemetry**: Hand-rolled pure SVG `PayloadCompositionDonut`, top 10 largest `PartitionSizeBarChart`, `CompressionEfficiencyGauge`, 1-click `PayloadPresetsCard`, interactive `PayloadArchitectureGuide` for CrAU/Protobuf/blobs.<br>• **Extractor & Partitions**: Filter chips, search, `PartitionTableList`, `ExtractionLiveCockpit`.<br>• **Source & Remote Loader**: Local dropzone and remote HTTP stream loader with device catalog presets.<br>• **Extracted Outputs & History**: `PayloadHistoryTab` output explorer. |
| **Utilities** | 5-tab Hardware Cockpit: `UtilitiesCockpitHero` (live ADB daemon port 5037 status, host toolchain versions, connected serial/mode, and server restart) + tabs:<br>• **Overview & Quick Actions**: `DeviceVitalsCard` 6-item live vitals, `InstantActionsCard` symmetrical 3x2 command grid, `AdbTransportGuideCard` interactive socket architecture guide.<br>• **Power & Tweaks**: `TargetRebootCard` symmetrical 3x2 reboot grid, `SystemTweaksCard` window animations, dark mode, DPI.<br>• **Diagnostics**: `ScreenshotStudioCard` frame capture, `LogcatStreamCard` real-time log buffer filter.<br>• **Fastboot**: `FastbootSlotControlCard` active slot switcher & power, `FastbootGetVarCard` deep `getvar all` variable inspector.<br>• **Host Setup**: `HostServerControlCard` server controls, `HostGoogleSetupCard` official Google platform-tools & USB driver installer. |
