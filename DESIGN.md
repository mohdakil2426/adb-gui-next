...
| **Payload Dumper** | 4-tab Hardware Cockpit: `PayloadDumperHeroBanner` (format detection, uncompressed footprint, partition counts, folder reveal) + tabs:<br>• **Overview**: Clean overview screen.<br>• **Extractor & Partitions**: Filter chips, search, `PartitionTableList`, destination selector, batch action footer.<br>• **Source & Remote Loader**: Local dropzone and remote HTTP stream loader with device catalog presets.<br>• **Extracted Outputs & History**: `PayloadHistoryTab` output explorer. |
