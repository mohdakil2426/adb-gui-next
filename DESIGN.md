...
| **Marketplace** | 4-tab Hardware Cockpit: `MarketplaceHeroBanner` (pulsating sync badge, device compatibility indicator, total apps, source repository badges) + tabs:<br>• **Overview**: pure SVG `SourceCompositionDonut`, `CategoryDistributionMeter`, `MarketplaceGuideCard` ASCII sideloading pipeline.<br>• **Browse & Search**: Search bar, `FilterBar` (chips, sources, sort), grid/list switcher, in-place `AppDetailView`.<br>• **Installed & Updates**: `AppUpdateRow` with 1-click **Update All** batch runner and release changelog snippets.<br>• **Sources & Repos**: `RepositorySourcesCard`, `GithubAuthCard` PAT rate limits and device-flow auth. |
