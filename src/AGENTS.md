...
`MemorySparkline` (polyline + gradient + gridlines), `PackageCompositionDonut` (`stroke-dasharray` arcs), `PartitionSizeBarChart` (CSS grid horizontal bars), `BatteryGauge` (tick bezel), `UsageBar` / `MicroScale` (scaleX fills), `StorageAllocationBar`, `PostureSpectrum`, `AppsCompositionDonut`, `SourceCompositionDonut`, `CategoryDistributionMeter`, `AvdResourceAllocationMeter`, `DiskUsageBreakdownChart`, `BandwidthGauge`, and `PartitionHierarchySvg` are all hand-rolled pure SVG. Any library that writes to a built-in prototype at import time (`recharts` → `decimal.js-light`) is strictly prohibited and incompatible with `freezePrototype: true` in `src-tauri/tauri.conf.json`.
...
| Dashboard | Telemetry auto-loads on device selection (no "click refresh to load" dead end); formatting stays in the frontend (`shared/utils/format.ts`) — the backend returns numbers; `AppsPanel` shares the `appOverviewTelemetry` query key (30 s stale) with App Manager overview so the adb round-trip runs once per device |
