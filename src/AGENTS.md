...
`MemorySparkline` (polyline + gradient), `PackageCompositionDonut` (`stroke-dasharray` arcs), `PartitionSizeBarChart` (CSS grid horizontal bars), `BatteryGauge` and `UsageBar`, `SourceCompositionDonut`, `CategoryDistributionMeter`, `AvdResourceAllocationMeter`, `DiskUsageBreakdownChart`, `BandwidthGauge`, and `PartitionHierarchySvg` are all hand-rolled pure SVG. Any library that writes to a built-in prototype at import time (`recharts` → `decimal.js-light`) is strictly prohibited and incompatible with `freezePrototype: true` in `src-tauri/tauri.conf.json`.
...
| Payload Dumper | 4-tab Hardware Cockpit (Overview & Telemetry, Extractor & Partitions, Source & Remote Loader, Extracted Outputs & History); `PayloadDumperHeroBanner` with format detection & uncompressed footprint; hand-rolled pure SVG `PayloadCompositionDonut`, `PartitionSizeBarChart`, and compression efficiency gauge; `ExtractionLiveCockpit` animated progress waves |
| Utilities | 5-tab Hardware Cockpit (Overview & Quick Actions, Power & Tweaks, Diagnostics, Fastboot, Host Setup); `UtilitiesCockpitHero` with live ADB daemon status; `DeviceVitalsCard` 6-item live vitals; `InstantActionsCard` symmetrical 3x2 grid; `AdbTransportGuideCard` socket architecture guide; deep `getvar all` variable inspector; official Google platform-tools & USB driver installer for Windows |
