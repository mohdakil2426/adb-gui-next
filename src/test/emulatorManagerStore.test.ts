...
describe("emulatorManagerStore", () => {
  ...
  it("tracks root wizard source selection", () => {
    ...
    expect(useEmulatorManagerStore.getState().rootWizard.step).toBe(
      "preflight"
    );
  });
  ...
  it("tracks root wizard progress", () => {
    ...
    expect(useEmulatorManagerStore.getState().rootWizard.progress?.step).toBe(
      3
    );
  });
  ...
  it("stores and clears restore plan", () => {
    ...
    expect(useEmulatorManagerStore.getState().restorePlan?.source).toBe(
      "Pixel_8_API_34"
    );
