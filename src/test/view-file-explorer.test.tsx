...
import { ViewFileExplorer } from "@/features/file-explorer/file-explorer-view";
...
const verifyFileRootAccessMock = vi.fn<(...args: unknown[]) => Promise<unknown>>();
...
vi.mock(import("@tanstack/react-virtual"), () => ({
  useVirtualizer: () => ({
    ...
    measureElement: vi.fn<() => void>(),
  }),
}));

vi.mock(import("@/desktop/backend"), () => ({
  CreateDirectory: vi.fn<() => Promise<unknown>>(),
  CreateFile: vi.fn<() => Promise<unknown>>(),
  DeleteFiles: vi.fn<() => Promise<unknown>>(),
  HostPathKinds: vi.fn<() => Promise<unknown>>(),
  ListFiles: (...args: unknown[]) => listFilesMock(...args),
  OpenDeviceFileInEditor: vi.fn<() => Promise<unknown>>(),
  PullFile: vi.fn<() => Promise<unknown>>(),
  PushFile: vi.fn<() => Promise<unknown>>(),
  RenameFile: vi.fn<() => Promise<unknown>>(),
  RevealDevicePathInExplorer: vi.fn<() => Promise<unknown>>(),
  SelectDirectoryForPull: vi.fn<() => Promise<unknown>>(),
  SelectDirectoryToPush: vi.fn<() => Promise<unknown>>(),
  SelectFileToPush: vi.fn<() => Promise<unknown>>(),
  SelectSaveDirectory: vi.fn<() => Promise<unknown>>(),
  TransferDeviceFiles: vi.fn<() => Promise<unknown>>(),
...
describe(ViewFileExplorer, () => {
  ...
  it("keeps scroll region layout classes on the file list", async () => {
    render(<ViewFileExplorer activeView="files" />);
  ...
  it("renders toolbar action buttons and tree items in the file explorer", () => {
    render(<ViewFileExplorer activeView="files" />);
    ...
    expect(screen.getByRole("button", { name: /^New/u })).toBeInTheDocument();
    expect(screen.getByRole("treeitem", { name: /Internal storage/u })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Download" })).toBeInTheDocument();
  });

  it("renders common folder bookmarks in the file explorer", () => {
    render(<ViewFileExplorer activeView="files" />);
  ...
  it("renders column sort buttons in the file explorer", async () => {
    render(<ViewFileExplorer activeView="files" />);

    await expect(
      screen.findByRole("button", { name: /Date modified/u })
    ).resolves.toBeInTheDocument();
    await expect(screen.findByRole("button", { name: /^Type/u })).resolves.toBeInTheDocument();
  });
  ...
  it("clears a row selection on empty list click and never shows a selection summary bar", async () => {
    ...
    expect(screen.queryByText(/items selected/u)).not.toBeInTheDocument();
  ...
  it("shows the no-device state from the device store, not from adb error text", async () => {
    ...
    await expect(screen.findByText("No device connected")).resolves.toBeInTheDocument();
    expect(screen.getByText(/Connect a device over USB/u)).toBeInTheDocument();
  });
