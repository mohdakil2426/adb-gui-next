...
const volume = (overrides: Partial<backend.StorageVolume>): backend.StorageVolume => ({
  freeBytes: 68 * GIB,
  mount: "/data",
  rawMount: "/data",
  totalBytes: 110 * GIB,
...
describe(StoragePanel, () => {
  ...
  it("shows an honest empty state instead of an empty card when every volume was filtered out", () => {
    ...
    expect(screen.getByText(/no user-facing storage volumes were reported/iu)).toBeInTheDocument();
  ...
  it("shows loading skeletons instead of the empty state while the first load is in flight", () => {
    ...
    expect(
      screen.queryByText(/no user-facing storage volumes were reported/iu)
