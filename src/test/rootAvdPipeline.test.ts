...
describe("rootAVD-aligned ramdisk patch pipeline", () => {
  it("creates ramdisk.cpio.orig before magiskboot backup uses it", () => {
    ...
    const createOrig =
      "cp {ROOT_WORKDIR}/ramdisk.cpio {ROOT_WORKDIR}/ramdisk.cpio.orig";
  ...
  it("does not reject API 30 plus multi-cpio ramdisks before magiskboot validation", () => {
    ...
    expect(source).not.toContain(
      "add the rootAVD repack path before proceeding"
    );
  });
  ...
  it("rebuilds API 30 plus multi-cpio ramdisks before Magisk patching", () => {
    ...
    expect(source).toContain(
      "find . | ../busybox cpio -H newc -o > ../ramdisk.cpio.repacked"
    );
    expect(source.indexOf(repackCall)).toBeLessThan(
      source.indexOf(magiskbootTest)
    );
    expect(source.indexOf(repackCall)).toBeLessThan(
      source.indexOf(patchCommand)
    );
  });
