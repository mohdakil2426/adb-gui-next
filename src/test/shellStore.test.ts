...
describe("shellStore", () => {
  ...
  describe("addHistoryEntry", () => {
    ...
    it("should add multiple entries to history", () => {
      useShellStore
        .getState()
        .addHistoryEntry({ type: "command", text: "cmd1" });
      useShellStore
        .getState()
        .addHistoryEntry({ type: "result", text: "result1" });
      useShellStore
        .getState()
        .addHistoryEntry({ type: "command", text: "cmd2" });
  ...
  describe("setHistory", () => {
    it("should replace entire history", () => {
      useShellStore
        .getState()
        .addHistoryEntry({ type: "command", text: "existing" });
    ...
    it("should clear history when set to empty array", () => {
      useShellStore
        .getState()
        .addHistoryEntry({ type: "command", text: "test" });
      useShellStore.getState().setHistory([]);
  ...
  describe("commandHistory integration", () => {
    it("should keep command history separate from display history", () => {
      useShellStore
        .getState()
        .addHistoryEntry({ type: "command", text: "display cmd" });
