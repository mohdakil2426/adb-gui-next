...
import { RemoteLoadProgressCard } from "@/features/payload-dumper/ui/remote-load-progress-card";
...
describe(RemoteLoadProgressCard, () => {
  it("renders stages and not-full-download copy", () => {
    ...
    expect(screen.getByText(/not downloading full 3\.52 GB/iu)).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /cancel loading partitions/iu })).toBeInTheDocument();
  });
  ...
  it("invokes onCancel when cancel is pressed", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /cancel loading partitions/iu }));
