...
import { InstalledAppsTab } from "@/features/app-manager/debloater/ui/installed-apps-tab";
import { useDeviceStore } from "@/shared/stores/device-store";

const packageLifecycleOpMock = vi.fn<(...args: unknown[]) => Promise<unknown>>();
const pullPackageApkMock = vi.fn<(...args: unknown[]) => Promise<unknown>>();
const selectSaveDirectoryMock = vi.fn<(...args: unknown[]) => Promise<unknown>>();
const uninstallPackageMock = vi.fn<(...args: unknown[]) => Promise<unknown>>();
...
describe(InstalledAppsTab, () => {
  ...
  it("renders toolbar filter pills with accurate package counts", () => {
    ...
    render(
      <InstalledAppsTab
        hasLoaded={true}
        loadError={null}
        onInspect={vi.fn<(pkg: string) => void>()}
        onRefresh={vi.fn<() => void>()}
      />
    );

    const allBtn = screen.getByRole("button", { name: /^all/iu });
    expect(allBtn).toHaveTextContent("3");
    const userBtn = screen.getByRole("button", { name: /^user/iu });
    expect(userBtn).toHaveTextContent("2");
    const sysBtn = screen.getByRole("button", { name: /^system/iu });
    expect(sysBtn).toHaveTextContent("1");
    const disBtn = screen.getByRole("button", { name: /^disabled/iu });
  ...
  it("filters package list based on search query", async () => {
    ...
    const searchInput = screen.getByPlaceholderText(/search apps by name or package id/iu);
  ...
  it("renders Target SDK and size metrics on rows", () => {
    ...
    expect(screen.getByText(/API 3[0-5]/u)).toBeInTheDocument();
    expect(screen.getByText(/MB/u)).toBeInTheDocument();
  });

  it("triggers inline quick actions (Launch, Force Stop, Inspect)", async () => {
    const user = userEvent.setup();
    const onInspectMock = vi.fn<(pkg: string) => void>();
  ...
  it("displays floating batch bar when packages are selected", async () => {
    const user = userEvent.setup();
    ...
    const row = screen.getByText("Camera Pro").closest('[role="treeitem"], [role="option"]');
    ...
    expect(screen.getByRole("button", { name: /export apks/iu })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /clear cache/iu })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /uninstall/iu })).toBeInTheDocument();
  });

  it("triggers batch force stop from batch bar", async () => {
    const user = userEvent.setup();
    packageLifecycleOpMock.mockResolvedValue("ok");
