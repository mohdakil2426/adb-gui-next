...
vi.mock("@/lib/desktop/backend", () => ({
  ...
  ScanAvdRootReadiness: (...args: unknown[]) =>
    scanAvdRootReadinessMock(...args),
  SelectPatchedRootImageFile: (...args: unknown[]) =>
    selectPatchedRootImageFileMock(...args),
  SelectRootPackageFile: (...args: unknown[]) =>
    selectRootPackageFileMock(...args),
  StopAvd: vi.fn(),
...
describe("RootWizard", () => {
  ...
  it("verifies root after patch installation before showing verified success", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: /verify root/i })
    );

    expect(await screen.findByText("Root Verified")).toBeInTheDocument();
    expect(verifyAvdRootMock).toHaveBeenCalledWith(
      runningAvd.name,
      runningAvd.serial
    );
  });
  ...
  it("exposes the FAKEBOOTIMG manual flow and finalizes a patched fake boot image", async () => {
    ...
    selectRootPackageFileMock.mockResolvedValue(
      "C:/Downloads/Magisk-v30.0.apk"
    );
    prepareAvdRootMock.mockResolvedValue({
      ...
      instructions: [
        "Open Magisk",
        "Patch /sdcard/Download/fakeboot.img",
        "Return and finalize",
      ],
    });
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: /manual mode/i })
    );
    expect(
      await screen.findByText("Manual Mode (FAKEBOOTIMG)")
    ).toBeInTheDocument();

    await userEvent.click(
      screen.getByRole("button", { name: /choose magisk package/i })
    );
    await userEvent.click(
      screen.getByRole("button", { name: /create fakeboot/i })
    );
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /finalize root/i })
    );
    ...
    expect(
      await screen.findByText("Manual Patch Installed")
    ).toBeInTheDocument();
  });
  ...
  it("finalizes manual root with a user-selected local patched image", async () => {
    ...
    selectPatchedRootImageFileMock.mockResolvedValue(
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /select patched image/i })
    );
    expect(
      await screen.findByText("C:/Downloads/magisk_patched-123.img")
    ).toBeInTheDocument();
  ...
  it("finalizes manual root with a dropped local patched image", async () => {
    ...
    expect(
      await screen.findByText("C:/Downloads/magisk_patched-dropped.img")
    ).toBeInTheDocument();
