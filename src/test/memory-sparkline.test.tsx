...
import { MemorySparkline } from "@/features/dashboard/ui/memory-sparkline";

const GIB = 1024 ** 3;

const sample = (offsetMs: number, usedGib: number): MemorySample => ({
  at: 1_800_000_000_000 + offsetMs,
  totalBytes: 8 * GIB,
...
describe(MemorySparkline, () => {
  it("plots one point per sample", () => {
    ...
    expect(polyline?.getAttribute("points")?.trim().split(/\s+/u)).toHaveLength(3);
  });
  ...
  it("reads out the latest sample before any hover", () => {
    ...
    expect(getByText(/6\.0 GB/u)).toBeInTheDocument();
    expect(getByText(/75(\.0)?%/u)).toBeInTheDocument();
  });
