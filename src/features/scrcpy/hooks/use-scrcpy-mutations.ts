...
export const openInstalledFolder = async (binaryPath?: string | null) => {
  if (!binaryPath) {
    return;
  }
  const folder = binaryPath.replace(/[/\\][^/\\]+$/u, "");
  try {
...
export const useScrcpyMutations = (options: backend.ScrcpyLaunchOptions) => {
  ...
  const updateActiveSerials = (updater: (prev: string[]) => string[]) => {
    queryClient.setQueryData(
      ...
      (prev: backend.ScrcpyActiveSessions | undefined) => {
        ...
        return {
          serials: next,
          sessions: next.map((s) => ({ pid: 0, serial: s })),
        };
      }
    );
  };
  ...
  const launch = useMutation({
    ...
    onSuccess: (_, serials) => {
      updateActiveSerials((existing) => [...new Set([...existing, ...serials])]);
      toast.success(
        serials.length > 1
