...
export const destructiveLaunchOptions = (
  options: backend.EmulatorLaunchOptions
): LaunchOptionMeta[] =>
  LAUNCH_OPTIONS.filter((option) => option.destructive && options[option.key]);
 ...
 */
export const unacknowledgedLaunchOptions = (
  options: backend.EmulatorLaunchOptions,
  acknowledgements: LaunchAcknowledgements
): LaunchOptionMeta[] =>
  destructiveLaunchOptions(options).filter((option) => !acknowledgements[option.key]);
