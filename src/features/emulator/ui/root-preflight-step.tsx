...
const StatusIcon = ({ status }: { status: backend.CheckStatus }) => {
  switch (status) {
    case "pass": {
...
const CheckRow = ({ check }: { check: backend.ReadinessCheck }) => (
  <li
    className={cn(
...
const RecommendedAction = ({
  ...
  <div className="flex flex-col gap-2 rounded-md border border-border bg-surface-raised p-3">
    ...
    {action.type === "coldBoot" && (
      <>
        <p className="text-body text-foreground">
          This emulator loaded from a Quick Boot snapshot. Root changes can be lost when it saves a
          new snapshot on shutdown.
        </p>
        <Button
          className="w-fit"
    ...
    {action.type === "unsupported" && <p className="text-body text-destructive">{action.reason}</p>}
  </div>
);

const getScanSummaryMessage = (canProceed: boolean, hasWarnings: boolean) => {
  if (!canProceed) {
    return "One or more checks failed. Resolve them above, then rescan.";
  }
  if (hasWarnings) {
    return "Ready to proceed with warnings. Review the items above before continuing.";
  }
  return "All checks passed. You are ready to root.";
};

const renderScanContent = (isScanning: boolean, scan: backend.RootReadinessScan | null) => {
  if (isScanning) {
    return (
...
export const RootPreflightStep = ({
  scan,
  isScanning,
  avdName,
  onRescan,
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {renderScanContent(isScanning, scan)}
      ...
      {!isScanning && scan ? (
        ...
        >
          {getScanSummaryMessage(canProceed, Boolean(scan.hasWarnings))}
        </p>
      ) : null}
