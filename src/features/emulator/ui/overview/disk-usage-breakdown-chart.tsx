...
const buildPartitions = (
  breakdown: backend.AvdDiskBreakdown | undefined,
  avd: backend.AvdSummary | null
): { partitions: PartitionSegment[]; totalGb: number } => {
  const sysGb = breakdown?.systemSizeGb ?? ((avd?.apiLevel ?? 30) >= 33 ? 3.4 : 2.6);
  const dataGb = breakdown?.dataSizeGb ?? 6;
  ...
  let accumulatedX = 0;
  const partitions: PartitionSegment[] = rawPartitions.map((segment) => {
    const currentX = accumulatedX;
    accumulatedX += segment.percentage;
    return { ...segment, x: currentX };
  });

  return { partitions, totalGb };
};

export const DiskUsageBreakdownChart = ({ avd }: DiskUsageBreakdownChartProps) => {
  const [hoveredId, setHoveredId] = useState<string | null>(null);

  const { data: breakdown } = useQuery({
    enabled: Boolean(avd?.name),
    queryFn: () =>
      avd?.name ? EmulatorGetDiskBreakdown(avd.name) : Promise.reject(new Error("No AVD")),
    queryKey: ["emulator", "diskBreakdown", avd?.name],
    staleTime: 30_000,
  });

  const { partitions, totalGb } = buildPartitions(breakdown, avd);
  return (
