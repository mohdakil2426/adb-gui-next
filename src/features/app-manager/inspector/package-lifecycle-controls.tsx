...
export const PackageLifecycleControls = ({
  info,
  onRefresh,
  selectedSerial,
  ...
  const runOp = async (op: string, label: string) => {
    ...
    try {
      ...
      toast.error(`Failed to ${label.toLowerCase()}: ${String(error)}`);
    } finally {
      setIsActing(false);
    }
  };
  ...
  const handlePullApk = async () => {
    ...
    try {
      ...
      toast.error(`Failed to export APK: ${String(error)}`);
    } finally {
      setIsActing(false);
    }
  };
  ...
  const handleOpenSettings = async () => {
    ...
    try {
      ...
      toast.error(`Failed to open settings: ${String(error)}`);
    }
  };
