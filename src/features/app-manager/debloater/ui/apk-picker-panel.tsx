...
} from "@/features/app-manager/debloater/model/installation-store";
import type { InstallProgress } from "@/features/app-manager/debloater/model/installation-store";
import { InstallDropZone } from "@/features/app-manager/debloater/ui/install-drop-zone";
import { InstallFlagsCockpit } from "@/features/app-manager/debloater/ui/install-flags-cockpit";
import { InstallProgressCard } from "@/features/app-manager/debloater/ui/install-progress-card";
import { PreFlightApkCard } from "@/features/app-manager/debloater/ui/pre-flight-apk-card";
...
export const ApkPickerPanel = ({
  ...
  useEffect(() => {
    ...
    const runBatchInspect = async () => {
      try {
        const results = await BatchInspectPackages(uninspected);
        if (!isCancelled) {
            ...
            >
              {isInstalling ? (
                ...
                <>
                  ...
                  <span>
                    Install {apkPaths.length} Package
                    {apkPaths.length === 1 ? "" : "s"} onto Device
                  </span>
                </>
              )}
            </Button>
