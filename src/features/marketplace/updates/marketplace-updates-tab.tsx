...
import { AppUpdateRow } from "@/features/marketplace/updates/app-update-row";
import type { AppUpdateItem } from "@/features/marketplace/updates/app-update-row";
import { UpdatesSummaryBanner } from "@/features/marketplace/updates/updates-summary-banner";
import { useDeviceStore } from "@/shared/stores/device-store";
import { Button } from "@/shared/ui/button";
import { Input } from "@/shared/ui/input";
import { runSerial } from "@/shared/utils/serial-async";

export const MarketplaceUpdatesTab = ({ target }: { target: InstallTarget }) => {
  ...
  const handleUpdate = async (item: AppUpdateItem) => {
    ...
    try {
      ...
      setUpdateStates((prev) => ({ ...prev, [item.packageName]: "error" }));
      toast.error(`Failed to update ${item.name}: ${String(error)}`, {
        id: toastId,
      });
    }
  };
