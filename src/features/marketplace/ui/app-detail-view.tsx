...
import { AppDetailHero } from "@/features/marketplace/ui/app-detail/app-detail-hero";
import { AppDetailSidebar } from "@/features/marketplace/ui/app-detail/app-detail-sidebar";
import { AppDetailSkeleton } from "@/features/marketplace/ui/app-detail/app-detail-skeleton";
import { AppDetailVersions } from "@/features/marketplace/ui/app-detail/app-detail-versions";
import { AppScreenshots } from "@/features/marketplace/ui/app-detail/app-screenshots";
...
interface AppDetailBodyProps {
  ...
  onRetry: () => void;
  primaryInstallState: InstallState;
  selectedApp: backend.MarketplaceApp;
  target: InstallTarget;
}
const AppDetailSidebarContainer = ({
  activeVersionName,
  canInstall,
  detail,
  isPrimaryInstalling,
  ...
  <div className="flex flex-col gap-5">
    ...
    {detail?.versions && detail.versions.length > 0 ? (
      <AppDetailVersions
        ...
        isPrimaryInstalling={isPrimaryInstalling}
...
const resolveAppDetailStrings = (
  detail: AppDetail | null,
  selectedApp: backend.MarketplaceApp,
  target: InstallTarget
) => {
  const targetPackageName = detail?.packageName ?? selectedApp.packageName;
  const displayName = detail?.name ?? selectedApp.name;
  ...
  const downloadsLabel = formatDownloadCount(
    ...
    <div className="grid @2xl:grid-cols-[1fr_280px] gap-6">
      <div className="flex min-w-0 flex-col gap-5">
        ...
        {detail?.readmeMarkdown ? (
          <section className="flex flex-col gap-2">
            <h2 className="text-caption text-muted-foreground uppercase tracking-wide">README</h2>
...
export const AppDetailView = ({ target }: { target: InstallTarget }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const fetchDetail = async () => {
        ...
        try {
          const nextDetail = await MarketplaceGetAppDetail(
  ...
  const {
    ...
    effectiveDownloadUrl,
    targetPackageName,
  } = resolveAppDetailStrings(detail, selectedApp, target);
    ...
    <div className="flex flex-col gap-5">
      ...
      {isLoadingDetail && !detail ? (
        ...
        <AppDetailBody
          ...
          primaryInstallState={primaryInstallState}
          selectedApp={selectedApp}
          target={target}
        />
      )}
