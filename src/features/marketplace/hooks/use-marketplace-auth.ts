    ...
    const pollHandler = {
      poll: async () => {
        try {
          ...
          if (result.status === "authorized" && result.accessToken) {
            ...
            try {
              await MarketplaceSavePat(result.accessToken);
              await refreshSecureAuth();
            } catch {
              // Ignore keychain persistence failure in polling
            }
            setGithubDeviceChallenge(null);
            setIsGithubAuthenticating(false);
          ...
          if (result.status === "authorization_pending") {
            ...
            pollHandler.schedulePoll(nextIntervalMs);
            return;
          }
          ...
          if (result.status === "access_denied" || result.status === "expired_token") {
            ...
            toast.error(
              result.status === "access_denied"
                ...
                : "GitHub code expired",
              {
                description: "Please try signing in again and approve the prompt promptly.",
              }
            );
          }
        } catch (error) {
      ...
      schedulePoll: (delayMs: number) => {
        ...
        timeoutRef.current = window.setTimeout(() => {
          timeoutRef.current = null;
          void pollHandler.poll();
        }, delayMs);
      },
    };

    pollHandler.schedulePoll(githubDeviceChallenge.challenge.interval * 1000);
