...
import { useMarketplaceAuth } from "@/features/marketplace/hooks/use-marketplace-auth";
...
interface GithubDeviceChallengeViewProps {
  challenge: {
    challenge: {
      expiresIn: number;
      userCode: string;
...
const GithubDeviceChallengeView = ({ challenge, onCancel }: GithubDeviceChallengeViewProps) => (
  <div className="mt-3 rounded-lg border bg-background/80 p-3 text-caption text-muted-foreground">
    <p className="font-medium text-foreground">Enter this code at github.com/login/device</p>
    <div className="mt-1 flex items-center gap-2">
      <span className="select-all font-mono text-foreground text-title tracking-[0.2em]">
        {challenge.challenge.userCode}
      </span>
      <CopyButton
        aria-label="Copy code"
        className="size-7"
        label="Device code"
        value={challenge.challenge.userCode}
      />
    </div>
    <p className="mt-1 text-caption">
      Code expires in {Math.ceil(challenge.challenge.expiresIn / 60)} min — approve in browser to
      finish.
    </p>
    <div className="mt-3 flex flex-wrap gap-2">
      <Button
        onClick={() => {
          openExternal(
            challenge.challenge.verificationUriComplete ?? challenge.challenge.verificationUri
          );
        }}
        size="sm"
        type="button"
    ...
    <div className="flex flex-col gap-2 rounded-lg border border-border/80 bg-surface-raised/40 p-3.5">
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          onClick={() => onSavePat(localPat.trim())}
          type="button"
        >
          Save Token
        </Button>
...
interface GithubKeychainSectionProps {
  authRateLimit: { limit: number; remaining: number; secondsUntilReset: number } | null | undefined;
  ...
  signOutGithub: () => void;
  tokenStatus:
    | { hasToken: boolean; login?: string | null; source?: string | null }
    | null
    | undefined;
}

const GithubKeychainSection = ({
  authRateLimit,
  cancelGithubSignIn,
  githubDeviceChallenge,
  ...
  <div className="rounded-lg border bg-muted/20 p-4">
    ...
    {isSignedIn ? (
      <div className="mt-3 rounded-lg border bg-background/80 p-3 text-caption text-muted-foreground">
        <div className="flex items-center gap-2 text-foreground">
          ...
          {tokenStatus?.source ? (
            <span className="text-caption text-muted-foreground">via {tokenStatus.source}</span>
          ) : null}
        </div>
        {authRateLimit ? (
          <p className="mt-2">
    ...
    {githubDeviceChallenge && isGithubAuthenticating ? (
      <GithubDeviceChallengeView challenge={githubDeviceChallenge} onCancel={cancelGithubSignIn} />
    ) : null}
    ...
    <Field className="mt-4">
      <FieldLabel htmlFor="github-oauth-client-id">
        GitHub OAuth client ID <span className="text-muted-foreground">(advanced, optional)</span>
      </FieldLabel>
      <Input
        autoComplete="off"
        ...
        onBlur={() => onSaveClientId(localClientId.trim())}
        onChange={(event) => {
      ...
      />
      <p className="text-caption text-muted-foreground">
        Leave empty for one-click sign-in. Custom ID only if you use your own GitHub OAuth app.
      </p>
    </Field>
  </div>
);
export const GithubAuthCard = () => {
  ...
  const handleSavePat = (pat: string) => {
    setGithubPat(pat);
  ...
  return (
    <Card className="rounded-xl border-border bg-surface shadow-none">
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <GithubKeychainSection
          authRateLimit={authRateLimit}
          cancelGithubSignIn={cancelGithubSignIn}
          githubDeviceChallenge={githubDeviceChallenge}
          isGithubAuthenticating={isGithubAuthenticating}
          isSignedIn={isSignedIn}
          localClientId={localClientId}
          login={login}
          onSaveClientId={setGithubOauthClientId}
          onStartGithubSignIn={handleStartGithubSignIn}
          setLocalClientId={setLocalClientId}
          signOutGithub={signOutGithub}
        ...
        <GithubPatSection githubPat={githubPat} onSavePat={handleSavePat} />
        {/* Rate Limit Readout */}
        {rateLimit ? (
