...
export const getFileAccessModeForPath = (
  path: string,
...
export const usePathFileAccessMode = (rootAccessGrantedRef: React.RefObject<boolean>) =>
  useCallback(
...
export const useFileExplorerRootAccess = (options: Options) => {
  const {
    activeView,
    currentPathRef,
    loadFiles,
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        const message = await VerifyFileRootAccess(selectedSerial);
        if (cancelled) {
          return;
        }
