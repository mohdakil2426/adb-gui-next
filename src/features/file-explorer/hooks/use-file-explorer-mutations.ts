...
import type { CreatingType } from "@/features/file-explorer/model/file-explorer-types";
...
export const useFileExplorerMutations = (options: Options) => {
  ...
  const handleCreateConfirm = useCallback(async () => {
    ...
    try {
      ...
      await (o.creatingType === "file"
        ? CreateFile(fullPath, serial, accessMode)
        : CreateDirectory(fullPath, serial, accessMode));
      o.setCreatingType(null);
      void o.loadFiles(basePath, false);
    } catch (error) {
      handleError(o.creatingType === "file" ? "Create File" : "Create Folder", error);
    } finally {
