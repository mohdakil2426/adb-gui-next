...
import { useFileExplorerClipboard } from "@/features/file-explorer/hooks/use-file-explorer-clipboard";
import { useFileExplorerHostDrop } from "@/features/file-explorer/hooks/use-file-explorer-host-drop";
import { useFileExplorerKeyboardShortcuts } from "@/features/file-explorer/hooks/use-file-explorer-keyboard-shortcuts";
import { useFileExplorerLayout } from "@/features/file-explorer/hooks/use-file-explorer-layout";
import { useFileExplorerLoader } from "@/features/file-explorer/hooks/use-file-explorer-loader";
import { useFileExplorerMutations } from "@/features/file-explorer/hooks/use-file-explorer-mutations";
import { useFileExplorerPathActions } from "@/features/file-explorer/hooks/use-file-explorer-path-actions";
import {
  useFileExplorerRootAccess,
  usePathFileAccessMode,
} from "@/features/file-explorer/hooks/use-file-explorer-root-access";
import { useFileExplorerSelection } from "@/features/file-explorer/hooks/use-file-explorer-selection";
import { useFileExplorerSort } from "@/features/file-explorer/hooks/use-file-explorer-sort";
import { useFileExplorerTransfers } from "@/features/file-explorer/hooks/use-file-explorer-transfers";
import { PHANTOM_ROW_HEIGHT } from "@/features/file-explorer/model/file-explorer-constants";
import {
  fileReducer,
  initFileState,
  initUIState,
  uiReducer,
} from "@/features/file-explorer/model/file-explorer-reducers";
import type {
  CreatingType,
  FileEntry,
  FileExplorerActions,
...
import { categorizeError } from "@/features/file-explorer/utils/file-explorer-errors";
...
export const useFileExplorerViewModel = (activeView: string) => {
  ...
  const setFileList = useCallback(
    (v: FileEntry[]) => fileDispatch({ payload: v, type: "SET_FILES" }),
    []
  );
  const setCurrentPath = useCallback(
    (v: string) => fileDispatch({ payload: v, type: "SET_PATH" }),
    []
  );
  const setIsLoading = useCallback(
    (v: boolean) => fileDispatch({ payload: v, type: "SET_LOADING" }),
    []
  );
  const setLoadError = useCallback(
    (v: LoadError) => fileDispatch({ payload: v, type: "SET_LOAD_ERROR" }),
    []
  );
  const setTreeRefreshKey = useCallback(
    (v: number) => fileDispatch({ payload: v, type: "SET_TREE_REFRESH_KEY" }),
    []
  );
  const setSearchQuery = useCallback(
    (v: string) => fileDispatch({ payload: v, type: "SET_SEARCH_QUERY" }),
    []
  );
  const setNavHistory = useCallback(
    (v: string[]) => fileDispatch({ payload: v, type: "SET_NAV_HISTORY" }),
    []
  );
  const setHistoryIndex = useCallback(
    (v: number) => fileDispatch({ payload: v, type: "SET_HISTORY_INDEX" }),
    []
  );
  const setRenamingName = useCallback(
    (v: string | null) => uiDispatch({ payload: v, type: "SET_RENAMING_NAME" }),
    []
  );
  const setRenameValue = useCallback(
    (v: string) => uiDispatch({ payload: v, type: "SET_RENAME_VALUE" }),
    []
  );
  const setRenameError = useCallback(
    (v: string) => uiDispatch({ payload: v, type: "SET_RENAME_ERROR" }),
    []
  );
  const setIsRenaming = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_IS_RENAMING" }),
    []
  );
  const setDeleteDialogOpen = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_DELETE_DIALOG_OPEN" }),
    []
  );
  const setFilesToDelete = useCallback(
    (v: string[]) => uiDispatch({ payload: v, type: "SET_FILES_TO_DELETE" }),
    []
  );
  const setIsDeleting = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_IS_DELETING" }),
    []
  );
  const setCreatingType = useCallback(
    (v: CreatingType) => uiDispatch({ payload: v, type: "SET_CREATING_TYPE" }),
    []
  );
  const setCreateName = useCallback(
    (v: string) => uiDispatch({ payload: v, type: "SET_CREATE_NAME" }),
    []
  );
  const setCreateError = useCallback(
    (v: string) => uiDispatch({ payload: v, type: "SET_CREATE_ERROR" }),
    []
  );
  const setIsCreating = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_IS_CREATING" }),
    []
  );
  const setIsPushing = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_IS_PUSHING" }),
    []
  );
  const setIsPulling = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_IS_PULLING" }),
    []
  );
  const setLeftWidth = useCallback(
    (v: number | ((prev: number) => number)) => uiDispatch({ payload: v, type: "SET_LEFT_WIDTH" }),
    []
  );
  const setIsResizing = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_IS_RESIZING" }),
    []
  );
  const setIsTreeCollapsed = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_IS_TREE_COLLAPSED" }),
    []
  );
  const setIsEditingPath = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_IS_EDITING_PATH" }),
    []
  );
  const setEditPathValue = useCallback(
    (v: string) => uiDispatch({ payload: v, type: "SET_EDIT_PATH_VALUE" }),
    []
  );
  const setRootAccessGranted = useCallback(
    (v: boolean) => uiDispatch({ payload: v, type: "SET_ROOT_ACCESS_GRANTED" }),
    []
  );
