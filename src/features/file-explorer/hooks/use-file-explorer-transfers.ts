...
export const useFileExplorerTransfers = (options: Options) => {
  const {
    currentPath,
    getFileAccessMode,
    loadFiles,
  ...
  const executePush = useCallback(
    async (localPath: string, targetDir: string, serial: string | null) => {
      ...
      let toastId: string | number = "";
      try {
        const fileName = localPath.replaceAll("\\", "/").split("/").pop() ?? "";
  ...
  const handlePushFile = useCallback(async () => {
    ...
    const fileName = localPath.replaceAll("\\", "/").split("/").pop() ?? path.basename(localPath);
  ...
  const handlePushFolder = useCallback(async () => {
    ...
    try {
      ...
      const folderName =
        localFolderPath.replaceAll("\\", "/").split("/").pop() ?? path.basename(localFolderPath);
      ...
      try {
        ...
        await Promise.all(
          kinds.map((item) => {
            ...
            const fileName = item.path.replaceAll("\\", "/").split("/").pop() ?? "";
