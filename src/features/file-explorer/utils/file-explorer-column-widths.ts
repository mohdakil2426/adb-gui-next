...
export const clampColumn = (id: FileColumnId, value: number): number =>
  Math.min(MAX[id], Math.max(MIN[id], Math.round(value)));

export const applyColumnDelta = (
  widths: FileColumnWidths,
  id: FileColumnId,
  dx: number
): FileColumnWidths => ({ ...widths, [id]: clampColumn(id, widths[id] + dx) });

/** Pixel tracks plus a trailing `1fr` so extra pane width does not inflate Name. */
export const fileTableTemplate = (widths: FileColumnWidths): string =>
  `${widths.name}px ${widths.date}px ${widths.type}px ${widths.size}px minmax(0, 1fr)`;
