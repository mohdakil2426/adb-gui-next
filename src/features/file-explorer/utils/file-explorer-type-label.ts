...
const fileBaseName = (name: string): string => name.split("/").pop() ?? name;

const fileExtension = (base: string): string | null => {
...
export const fileTypeLabel = (file: Pick<FileEntry, "name" | "type">): string => {
  ...
  const namedBase = NAMED_BASENAMES[lower] ?? NAMED_BASENAMES[lower.replace(/^\./u, "")];
  if (namedBase) {
    return namedBase;
  }
