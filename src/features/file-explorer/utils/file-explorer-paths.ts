...
export const isValidDevicePath = (path: string | null): path is string => {
 ...
 */
export const toPathSegments = (currentPath: string): PathSegment[] => {
...
export const joinRemoteDir = (dir: string, name: string): string => {
  const trimmed = dir.trim();
  const base = trimmed === "/" ? "" : trimmed.replace(/\/+$/u, "");
  return `${base}/${name}`;
};

/** Destination path for copying `sourcePath` into `destDir`. */
export const destinationPath = (destDir: string, sourcePath: string): string =>
  joinRemoteDir(destDir, getFileName(sourcePath.replace(/\/+$/u, "")));
