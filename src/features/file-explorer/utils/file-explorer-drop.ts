...
export const parseInternalFileNames = (dataTransfer: DataTransfer | null): string[] | null => {
...
export const setDropOver = (element: Element | null, on: boolean): void => {
  if (!(element instanceof HTMLElement)) {
    return;
  }
  if (on) {
    element.dataset.feDropOver = "";
    return;
  }
  delete element.dataset.feDropOver;
};

export const closestFileExplorerDropTarget = (x: number, y: number): HTMLElement | null => {
...
export const destDirFromDropTarget = (
  ...
  return element.dataset.feDropDir ?? element.dataset.feDropPane ?? fallback;
};

const isInternalDrag = (dataTransfer: DataTransfer | null): boolean =>
  Boolean(dataTransfer?.types.includes(INTERNAL_FILES_MIME));

export const folderInternalDropProps = (
  destDir: string,
