...
import { ToolbarTooltip } from "@/features/file-explorer/ui/toolbar-tooltip";
import {
  FE_DROP_OVER_CLASS,
  folderInternalDropProps,
} from "@/features/file-explorer/utils/file-explorer-drop";
import { toPathSegments } from "@/features/file-explorer/utils/file-explorer-paths";
import type { PathSegment } from "@/features/file-explorer/utils/file-explorer-paths";
import {
  Breadcrumb,
  BreadcrumbEllipsis,
  BreadcrumbItem,
...
const collapse = (segments: PathSegment[]): CollapsedPath => {
...
const CrumbLabel = ({ isRoot, segment }: CrumbLabelProps) => {
  if (isRoot) {
    return (
      <>
...
const crumbDest = (path: string): string =>
...
const PathCrumb = ({ isLast, isRoot, onMoveToFolder, onNavigate, segment }: PathCrumbProps) => {
 ...
 */
export const FileExplorerPathBar = memo((props: Props) => {
  const {
    currentPath,
    editPathValue,
    isEditingPath,
                    ...
                    <BreadcrumbItem className="shrink-0">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="start">
                          <DropdownMenuGroup>
                            {hidden.map((hiddenSeg) => (
                              <DropdownMenuItem
                                key={hiddenSeg.path}
                                onClick={() => onNavigate(hiddenSeg.path)}
                              >
                                <span className="font-mono text-mono">{hiddenSeg.label}</span>
                              </DropdownMenuItem>
                            ))}
                          </DropdownMenuGroup>
                        </DropdownMenuContent>
