...
import { FileExplorerOpenWithMenuItems } from "@/features/file-explorer/ui/file-explorer-open-with-menu-items";
...
export const FileExplorerRowMenuItems = ({
  actions,
  currentPath,
  file,
  isBusy,
  ...
  const names = isSelected && selectedCount > 0 ? [...selectedNames] : [file.name];
    ...
    <>
      ...
      {isNavigable ? (
        <>
          <ContextMenuItem
            onClick={() => {
              void actions.loadFiles(`${path.posix.join(currentPath, file.name)}/`);
            }}
          >
      ...
      {file.type === "File" && isTextDeviceFile(file.name) ? (
        <FileExplorerOpenWithMenuItems
          disabled={isBusy}
          onOpenWith={(target) => {
            void actions.handleOpenInEditor(file, target);
          }}
        />
      ) : null}
      <ContextMenuItem
        disabled={isBusy}
        onClick={() => {
          void actions.handleShowInExplorer(file);
        }}
      >
        <FolderOpen aria-hidden="true" className="size-4 shrink-0" />
        Show in Explorer
      ...
      <ContextMenuItem
        ...
        onClick={() => {
          actions.openDeleteDialog(
            isSelected && selectedCount > 0 ? [...selectedNames] : [file.name]
          );
        }}
      >
      ...
      <ContextMenuItem
        disabled={isBusy}
        onClick={() => {
          void actions.handlePullItem(file);
        }}
      >
