...
const FileEntryIcon = ({ type }: { type: string }) => {
  if (type === "Directory") {
    return <Folder aria-hidden="true" className="size-4 shrink-0 text-primary" />;
  }
  if (type === "Symlink") {
    return <LinkIcon aria-hidden="true" className="size-4 shrink-0 text-info" />;
  }
  return <File aria-hidden="true" className="size-4 shrink-0 text-foreground-subtle" />;
};

export const FileExplorerRow = memo(
  ({
    currentPath,
    file,
    fileTableColumns,
