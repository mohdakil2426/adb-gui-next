 ...
 */
export const FileExplorerRowSkeleton = () => (
  <output aria-label="Loading directory…" className="block w-full">
    {SKELETON_ROWS.map((index) => (
      <div
        ...
        key={index}
        style={{
          gridTemplateColumns: FILE_TABLE_COLUMNS,
 ...
 */
const TableState = ({ action, description, icon: Icon, title, tone = "neutral" }: StateProps) => (
...
export const PermissionDeniedState = ({ rootAccessGranted }: PermissionDeniedProps) => (
  <TableState
    description={
      rootAccessGranted
...
export const LoadFailedState = ({ onRetry }: { onRetry: () => void }) => (
  <TableState
    action={
...
export const EmptyDirectoryState = ({
  disabled,
  onCreateFile,
  onCreateFolder,
}: EmptyDirectoryProps) => (
