...
const DeleteFileIcon = ({ type }: { type?: string | undefined }) => {
  if (type === "Directory") {
    return <Folder aria-hidden="true" className="size-3 shrink-0" />;
  }
  if (type === "Symlink") {
    return <LinkIcon aria-hidden="true" className="size-3 shrink-0" />;
  }
  return <File aria-hidden="true" className="size-3 shrink-0" />;
};
