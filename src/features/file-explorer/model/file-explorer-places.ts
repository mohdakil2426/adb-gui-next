...
export const normalizeDirPath = (path: string): string => {
  if (path === "/") {
    return "/";
  }
...
export const activePlaceId = (currentPath: string): string | null => {
  const current = normalizeDirPath(currentPath);
  let best: FileExplorerPlace | null = null;
  for (const place of FILE_EXPLORER_PLACES) {
    if (
      (current === place.path || current.startsWith(place.path)) &&
      (!best || place.path.length > best.path.length)
    ) {
      best = place;
    }
  }
