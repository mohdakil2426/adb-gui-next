import { getStoredRootAccessGranted } from "@/features/file-explorer/hooks/use-file-explorer-root-access";
import {
  DEFAULT_LEFT_WIDTH,
  MAX_LEFT_WIDTH,
  MIN_LEFT_WIDTH,
...
import { isValidDevicePath } from "@/features/file-explorer/utils/file-explorer-paths";
...
const handleDialogOrTransferAction = (state: UIState, action: UIAction): UIState | null => {
  switch (action.type) {
...
export const uiReducer = (state: UIState, action: UIAction): UIState => {
  const dialogOrTransfer = handleDialogOrTransferAction(state, action);
  if (dialogOrTransfer) {
    return dialogOrTransfer;
  }
  switch (action.type) {
