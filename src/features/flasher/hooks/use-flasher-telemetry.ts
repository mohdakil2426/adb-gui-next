...
const normalizeSlot = (raw?: string | null): ActiveSlot => {
  if (!raw) {
    return "unknown";
  }
  const clean = raw.toLowerCase().trim().replace(/^_+/u, "");
...
const parseLockState = (raw: string | null): BootloaderLockState => {
  if (!raw) {
    return "UNKNOWN";
  }
...
export const useFlasherTelemetry = () => {
  ...
  const rebootDevice = useCallback(
    async (mode: "system" | "bootloader" | "recovery" | "fastboot") => {
      ...
      try {
        const rebootTarget = mode === "system" ? "" : mode;
        await (isFastbootMode
          ? RunFastbootHostCommand(`reboot ${rebootTarget}`.trim(), selectedDevice.serial)
          : Reboot(rebootTarget, selectedDevice.serial));
