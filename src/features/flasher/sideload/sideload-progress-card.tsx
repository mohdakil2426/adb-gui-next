...
const STEP_BORDER_STYLES: Record<string, string> = {
  active: "border-info/40 bg-info/5 shadow-xs",
  complete: "border-success/30 bg-success/5",
};

const STEP_BADGE_VARIANTS: Record<string, "default" | "success" | "outline"> = {
  active: "default",
  complete: "success",
};
...
export const SideloadProgressCard = ({
  isSideloading,
  fileName,
  packagePath,
  ...
  useEffect(() => {
    ...
    const unlisten = EventsOn<backend.SideloadProgress>("flasher:sideload-progress", (payload) => {
      ...
      setStreamProgress({ message, percentage, stage });
    });
  ...
  if (streamProgress.stage === "verifying") {
    installStatus = "active";
  } else if (streamProgress.stage === "success") {
          ...
          <div className="flex flex-col gap-2 rounded-lg border border-info/30 bg-info/5 p-3">
            ...
            <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
              <div
                ...
                style={{
                  width: `${Math.max(2, Math.min(100, streamProgress.percentage))}%`,
                }}
              />
            </div>
          </div>
          ...
          {steps.map((step) => (
            <div
              className={cn(
                ...
                STEP_BORDER_STYLES[step.status] ?? "border-border/70 bg-surface-raised/40"
              )}
              key={step.id}
            >
              ...
              <Badge
                ...
                variant={STEP_BADGE_VARIANTS[step.status] ?? "outline"}
              >
                {step.status}
              </Badge>
            </div>
