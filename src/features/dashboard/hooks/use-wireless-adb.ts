...
import { useWirelessAdbStore } from "@/shared/stores/wireless-adb-store";
...
import { wirelessAdbSchema } from "@/shared/utils/schemas";
...
export const useWirelessAdb = (
  ...
  const ip = useWatch({ control: form.control, defaultValue: "", name: "ip" });
  const port = useWatch({
  ...
  const connect = useCallback(
    async (values: WirelessAdbValues) => {
      ...
      try {
        ...
        toast.success("Connected", { description: output, id: toastId });
  ...
  const disconnect = useCallback(async () => {
    ...
    try {
      ...
      toast.success("Disconnected", { description: output, id: toastId });
