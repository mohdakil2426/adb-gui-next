...
import { TONE_STROKE, TONE_TEXT } from "@/features/dashboard/model/tone";
...
const ChargingAura = ({
  ...
  <m.div
    animate={
      shouldReduceMotion
        ? { opacity: 0.1, scale: 1 }
        : { opacity: [0.08, 0.18, 0.08], scale: [1, 1.12, 1] }
    }
    aria-hidden="true"
    className={cn(
...
export const BatteryGauge = ({ isCharging, levelPct, tone }: BatteryGaugeProps) => {
  ...
  return (
    ...
    >
      {isCharging ? <ChargingAura shouldReduceMotion={shouldReduceMotion} tone={tone} /> : null}
      <svg aria-label={label} className="size-32 -rotate-90" viewBox={`0 0 ${SIZE} ${SIZE}`}>
        <BezelTicks />
        {/* Track */}
        <circle
        ...
        {levelPct === null ? null : (
          <m.circle
            ...
            transition={
              shouldReduceMotion
                ? { duration: 0 }
                : { delay: 0.1, duration: 0.8, ease: EASE_STANDARD }
            }
          />
        )}
