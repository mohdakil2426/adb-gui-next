...
import { TONE_FILL, TONE_TEXT } from "@/features/dashboard/model/tone";
...
export const MicroScale = ({
  ...
  const segments: { tone: Tone; width: number }[] = [];
  let cursor = min;
  for (const zone of zones) {
    const width = ((Math.min(zone.to, max) - cursor) / span) * 100;
    cursor = zone.to;
    segments.push({ tone: zone.tone, width });
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="relative h-1.5 w-full overflow-hidden rounded-full bg-secondary">
        <progress aria-label={ariaLabel} className="sr-only" max={max} value={value ?? undefined}>
          {display}
        </progress>
