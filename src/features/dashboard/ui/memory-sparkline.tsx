...
const buildPoints = (samples: MemorySample[]): Point[] => {
  if (samples.length === 0) {
    return [];
  }
  if (samples.length === 1) {
    ...
    return [
      { at: sample.at, ratio, usedBytes: sample.usedBytes, x: 0, y },
      { at: sample.at, ratio, usedBytes: sample.usedBytes, x: VIEW_W, y },
    ];
  }
...
export const MemorySparkline = ({ samples }: { samples: MemorySample[] }) => {
