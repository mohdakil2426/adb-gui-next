...
import { useAppOverview } from "@/features/dashboard/hooks/use-app-overview";
import { AppsCompositionDonut } from "@/features/dashboard/ui/apps-composition-donut";
import type { CompositionSlice } from "@/features/dashboard/ui/apps-composition-donut";
...
const SdkDistributionMeter = ({ buckets, total }: { buckets: SdkBuckets; total: number }) => {
...
const AppsErrorView = ({ error, onRetry }: { error: Error; onRetry: () => void }) => (
...
export const AppsPanel = ({ isEnabled, serial }: AppsPanelProps) => {
  ...
  const renderContent = () => {
    ...
    if (error && !telemetry) {
      return <AppsErrorView error={error} onRetry={refresh} />;
    }
