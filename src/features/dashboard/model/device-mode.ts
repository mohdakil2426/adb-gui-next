...
export const getDeviceMode = (device: backend.Device | null | undefined): DeviceMode => {
  if (!device) {
    return "unavailable";
  }
 ...
 */
export const supportsTelemetry = (device: backend.Device | null | undefined): boolean =>
  device?.status === "device";

/** `192.168.1.14:5555` — how `adb connect` names a wireless device. */
const WIRELESS_SERIAL = /^\d{1,3}(?:\.\d{1,3}){3}:\d{1,5}$/u;

export const isWirelessSerial = (serial: string | null | undefined): boolean =>
  Boolean(serial) && WIRELESS_SERIAL.test(serial ?? "");

/** Why telemetry is unavailable, phrased as the next thing to do. */
export const telemetryBlockedReason = (device: backend.Device | null | undefined): string => {
