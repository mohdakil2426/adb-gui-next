...
import type { ExtractionRecord } from "@/features/payload-dumper/model/payload-dumper-store";
...
const getStatusIconBorder = (status: string): string => {
  if (status === "success") {
...
const STATUS_BADGE_VARIANTS: Record<string, "default" | "outline" | "destructive"> = {
  cancelled: "outline",
  failed: "destructive",
  success: "default",
};
...
export const HistoryRecordCard = ({ record, onFlashToDevice }: HistoryRecordCardProps) => {
          ...
          <div className="flex items-center gap-2.5">
            <div
              className={cn(
                ...
                getStatusIconBorder(record.status)
              )}
            >
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={STATUS_BADGE_VARIANTS[record.status] ?? "destructive"}>
                  {STATUS_LABEL[record.status] ?? "Failed"}
                </Badge>
              </div>
              <span
