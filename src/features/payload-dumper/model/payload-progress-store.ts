...
export const usePayloadProgressStore = create<PayloadProgressState>((set) => ({
  ...
  clearTransientPartitionStatuses: () => {
    set((state) => ({
      partitionStatuses: keepCompletedOnly(state.partitionStatuses),
    }));
  },
  completedPartitions: new Set<string>(),
  extractingPartitions: new Set<string>(),
  ...
  markCompleted: (name) => {
    set((state) => {
      const updatedCompleted = new Set([...state.completedPartitions, name]);
  ...
  setPartitionStatus: (name, status) => {
    set((state) => {
      const statuses = new Map([...state.partitionStatuses, [name, status]]);
  ...
  updatePartitionProgress: (
    ...
    set((state) => {
      ...
      const statuses = new Map(state.partitionStatuses);
      let nextStatus: PartitionExtractStatus = "running";
      if (extractStatus) {
        nextStatus = extractStatus;
      } else if (completed || statuses.get(name) === "completed") {
