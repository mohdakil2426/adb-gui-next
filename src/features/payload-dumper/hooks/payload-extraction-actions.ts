...
} from "@/features/payload-dumper/model/payload-progress-store";
...
const isCancelledMessage = (message: string | null | undefined): boolean => {
  if (!message) {
    return false;
  }
...
const partitionNameFromExtractedFile = (fileName: string): string => {
  const base = fileName.split(/[/\\]/u).pop() ?? fileName;
  return base.replace(/\.img$/iu, "");
};

/** Best-effort stats from last progress samples when ExtractPayloadResult.stats is missing. */
const aggregateProgressStats = (partitionsExtracted: number): backend.ExtractionStats | null => {
...
const getPartitionsToExtract = (
  ...
  return partitionsToExtract;
};

const recordExtractionHistory = (
  result: backend.ExtractPayloadResult,
  newFiles: string[],
  outputDir: string,
  outputPath: string,
  payloadPath: string
  ...
  usePayloadDumperStore.getState().addToHistory({
    ...
    partitions: parts,
    payloadPath,
    status: resolveHistoryStatus(result.success, result.error),
    timestamp: Date.now(),
    totalBytes,
  });
};

export const runExtractPayload = async ({
  ...
  const partitionsToExtract = getPartitionsToExtract(partitions, completedPartitions);
  if (!partitionsToExtract) {
    return;
  }
  ...
  try {
    ...
    recordExtractionHistory(result, newFiles, outputDir, outputPath, payloadPath);
    if (result.success) {
      setStatus("success");
...
export const runResetPayloadDumper = (
  reset: () => void,
