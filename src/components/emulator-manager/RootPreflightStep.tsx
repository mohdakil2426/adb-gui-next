...
export function RootPreflightStep({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {!isScanning && recommendedAction ? (
        <div className="rounded-lg border border-border bg-muted/30 p-4">
          ...
          {recommendedAction.type === "launchEmulator" && (
            <div className="flex flex-col gap-2">
              ...
              <p className="text-muted-foreground text-xs">
                Cold Boot starts the emulator fresh without loading a saved
                state — strongly recommended for rooting.
              </p>
            </div>
          )}
          {recommendedAction.type === "coldBoot" && (
            <div className="flex flex-col gap-2">
              <p className="text-foreground text-sm">
                Your emulator loaded from a Quick Boot snapshot. Root changes
                may be lost if the emulator saves a new snapshot on shutdown.
              </p>
              <Button
              ...
              <p className="text-muted-foreground text-xs">
                You can still proceed without cold booting, but we recommend
                restarting to avoid snapshot overwrites.
              </p>
            </div>
          )}
          {recommendedAction.type === "restoreFirst" && (
            <div className="flex flex-col gap-2">
              <p className="text-foreground text-sm">
                The ramdisk appears to be modified. Restoring stock first gives
                you a clean slate.
              </p>
              <Button
      ...
      {/* Why info */}
      <p className="text-muted-foreground text-xs">
        ℹ️ Cold Boot starts the emulator without loading any saved state,
        ensuring root changes persist. Normal Boot loads a Quick Boot snapshot
        which may overwrite your root patch.
      </p>
    </div>
  );
}
