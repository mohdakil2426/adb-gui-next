...
import { Trash2, Copy, Logs, Save, X } from 'lucide-react';
...
export const TerminalLogPanel = () => {
  ...
  const handleCopy = () => {
    const text = logs
      .map(
        (l: { timestamp: string; type: string; message: string }) =>
            ...
            {logs.length > 0 && (
              <>
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-7 w-7 text-zinc-400 hover:text-red-400 hover:bg-red-900/30"
                  onClick={clearLogs}
                  title="Clear Logs"
                >
