...
function parseBuildFingerprint(
  fp: string
): { device: string; build: string } | null {
...
export function FileBannerDetails({
  ...
  const hasOtaPackageInfo =
    ...
    metadata.otaType ??
    metadata.otaVersion;
  const hasPayloadProperties =
    metadata.fileHash ?? metadata.fileSize ?? metadata.metadataHash;
  const buildInfo = metadata.postBuild
    ? parseBuildFingerprint(metadata.postBuild)
    : null;
    ...
    <div className="gap-4 border-border/50 border-t pt-3">
      ...
      {hasOtaPackageInfo ? (
        <div className="gap-2">
          ...
          <div className="gap-1.5 pl-1">
            {metadata.preDevice ? (
              <MetadataRow label="Device" value={metadata.preDevice} />
            ) : null}
            {metadata.postSdkLevel ? (
              <MetadataRow
                label="Android"
                value={sdkToAndroid(metadata.postSdkLevel)}
              />
            ) : null}
            {buildInfo ? (
              <MetadataRow
            ...
            {metadata.postSecurityPatchLevel ? (
              <MetadataRow
                ...
                value={metadata.postSecurityPatchLevel}
              />
            ) : null}
            {metadata.postTimestamp ? (
              <MetadataRow
                label="Build Date"
                value={formatTimestamp(metadata.postTimestamp)}
              />
            ) : null}
            {metadata.otaVersion ? (
              <MetadataRow
        ...
        <div className="gap-2">
          ...
          <div className="gap-1.5 pl-1">
            {metadata.fileSize !== null && (
              <MetadataRow
                label="Payload Size"
                value={formatBytesNum(metadata.fileSize)}
              />
            )}
            {metadata.fileHash ? (
              <MetadataRow
                label="File Hash"
                value={<CopyableText text={metadata.fileHash} />}
              />
            ) : null}
            {metadata.metadataSize !== null && (
              <MetadataRow
                label="Metadata Size"
                value={formatBytesNum(metadata.metadataSize)}
              />
            )}
      ...
      <div className="gap-2">
        ...
        <div className="gap-1.5 pl-1">
          <MetadataRow
            label="Full URL"
            value={<CopyableText text={remoteUrl} />}
          />
          <MetadataRow
            label="File Size"
            value={formatBytesNum(metadata.contentLength)}
          />
          {metadata.contentType ? (
            <MetadataRow label="Content-Type" value={metadata.contentType} />
          ) : null}
          {metadata.server ? (
            <MetadataRow label="Server" value={metadata.server} />
          ) : null}
        ...
        <div className="gap-2">
          ...
          <div className="gap-1.5 pl-1">
            ...
            {metadata.zipCompressionMethod ? (
              <MetadataRow
                label="Compression"
                value={metadata.zipCompressionMethod}
              />
            ) : null}
      ...
      <div className="gap-2">
        ...
        <div className="gap-1.5 pl-1">
          ...
          <MetadataRow
            label="Block Size"
            value={`${metadata.blockSize} bytes`}
          />
          <MetadataRow
            label="Update Type"
            value={formatUpdateType(metadata.minorVersion)}
          />
          {metadata.securityPatchLevel ? (
            <MetadataRow
              label="Security Patch"
              value={metadata.securityPatchLevel}
            />
          ) : null}
          <MetadataRow
            label="Timestamp"
            value={formatTimestamp(metadata.maxTimestamp)}
          />
          <MetadataRow
            label="Partial Update"
            value={metadata.partialUpdate ? "Yes" : "No"}
          />
        </div>
      </div>
      ...
      <div className="gap-2">
        ...
        <div className="gap-1.5 pl-1">
          <MetadataRow
            ...
            value={
              prefetch
                ? "Prefetch (download first)"
                : "Direct (HTTP range on-demand)"
            }
          />
          {outputPath ? (
            <MetadataRow
