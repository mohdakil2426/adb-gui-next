...
const readRecentUrlsFromStorage = (): string[] => {
  try {
    const saved = localStorage.getItem(STORAGE_KEY_V1) ?? localStorage.getItem(LEGACY_STORAGE_KEY);
    if (saved) {
...
export const RemoteUrlPanel = ({
  ...
  const [recentUrls, setRecentUrls] = useState<string[]>(readRecentUrlsFromStorage);
        ...
        <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-border/40 bg-surface px-2.5 py-1.5 text-[11px]">
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1">
            {recentUrls.map((recent) => {
              const label = getFileName(recent) || recent.replace(/^https?:\/\//u, "").slice(0, 30);
