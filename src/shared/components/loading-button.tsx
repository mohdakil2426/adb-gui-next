...
export const LoadingButton = ({
  ...
  <Button aria-busy={isLoading} disabled={disabled ?? isLoading} size={size} type={type} {...props}>
    {isLoading ? (
      <output className="contents">
        <Loader2 aria-hidden="true" className="size-4 animate-spin" data-icon="inline-start" />
      </output>
    ) : (
      icon
    )}
