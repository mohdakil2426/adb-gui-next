...
import { AVAILABLE, blocked, deviceLabel } from "@/shared/commands/types";
import type { CommandAction, CommandAvailability, CommandContext } from "@/shared/commands/types";
import { trackOperation } from "@/shared/stores/operation-store";
 ...
 */
export const deviceAvailability = (ctx: CommandContext): CommandAvailability => {
...
const runReboot = (ctx: CommandContext, target: RebootTarget): void => {
...
const copySerial = (ctx: CommandContext): void => {
  const serial = ctx.selectedSerial;
  if (!serial) {
    return;
  }
...
export const deviceActionCommands = (): CommandAction[] => {
 ...
 */
export const deviceSelectionCommands = (ctx: CommandContext): CommandAction[] => {
