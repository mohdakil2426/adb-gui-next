...
export const ipv4Schema = z
  ...
  .regex(/^(\d{1,3}\.){3}\d{1,3}$/u, "Must be a valid IPv4 address (e.g. 192.168.1.100)")
  .refine(
    (v) => v.split(".").every((octet) => Number.parseInt(octet, 10) <= 255),
    "Each octet must be 0–255"
  );

export const portSchema = z
  .string()
  .regex(/^\d{4,5}$/u, "Port must be 4–5 digits")
  .refine((v) => Number.parseInt(v, 10) <= 65_535, "Port must be ≤ 65535")
  .refine((v) => Number.parseInt(v, 10) >= 1024, "Port must be ≥ 1024");
