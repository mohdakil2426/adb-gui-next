...
export const startOperation = (input: StartOperationInput): string =>
  useOperationStore.getState().start(input);

export const updateOperation = (id: string, patch: OperationPatch): void => {
  useOperationStore.getState().update(id, patch);
};

export const finishOperation = (id: string): void => {
 ...
 */
export const useActiveOperation = (): Operation | null =>
  useOperationStore((state) => state.operations.at(-1) ?? null);

/** How many operations are queued behind the active one. */
export const usePendingOperationCount = (): number =>
  useOperationStore((state) => Math.max(state.operations.length - 1, 0));

/** Number of operations owned by `view` — drives the sidebar activity badge. */
export const useViewActivityCount = (view: ViewType): number =>
  useOperationStore((state) => {
    let total = 0;
    for (const operation of state.operations) {
      if (operation.view === view) {
        total += 1;
      }
    }
    return total;
