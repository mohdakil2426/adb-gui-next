...
import type { ShortcutHelp } from "@/shared/commands/shortcuts";
...
import { useNicknameStore } from "@/shared/stores/nickname-store";
import {
  Command,
  CommandDialog,
  CommandEmpty,
...
const SHORTCUT_GROUPS: { scope: string; entries: ShortcutHelp[] }[] = (() => {
  ...
  return Object.entries(groups).map(([scope, entries]) => ({
    entries,
    scope,
  }));
})();
...
const PaletteRow = ({ action, ctx, onRun }: PaletteRowProps) => {
...
export const CommandPalette = ({
  ...
  const groups = useMemo(() => {
    ...
    const result: ((typeof COMMAND_GROUPS)[number] & {
