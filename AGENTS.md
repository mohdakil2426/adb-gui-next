...
│   ├── App.tsx                   # Root → MainLayout (no router)
│   ├── styles/global.css         # Tailwind v4 config + shadcn theme tokens
│   ├── components/
...
│       ├── *Store.ts             # 3 Zustand stores (device, log, payloadDumper)
│       ├── nicknameStore.ts      # localStorage wrapper (NOT Zustand — no reactivity)
│       └── desktop/              # Tauri abstraction layer
│           ├── backend.ts        # 252 lines — every invoke<T>() wrapper
│           ├── runtime.ts        # 139 lines — event listeners, file drop, URL opener
│           └── models.ts         # 121 lines — DTO classes matching Rust structs
│
├── src-tauri/                    # Rust backend — Tauri 2
│   ├── src/
│   │   ├── lib.rs                # 833 lines — 26 Tauri commands + all helpers (MONOLITHIC)
│   │   ├── payload.rs            # 645 lines — OTA CrAU parser, decompress, SHA-256
│   │   └── generated/            # prost protobuf types (checked-in for IDE support)
│   ├── build.rs                  # prost-build + tauri-build
│   ├── Cargo.toml                # 16 deps (tauri, prost, zip, zstd, xz2, bzip2, sha2...)
│   ├── tauri.conf.json           # Window 1280×820, bundle targets: all
│   ├── tauri.windows.conf.json   # Windows-specific bundling
│   ├── tauri.linux.conf.json     # Linux-specific bundling
│   ├── capabilities/default.json # Tauri capability grants (core + opener)
│   ├── update_metadata.proto     # Chrome OS update engine protobuf schema
│   └── resources/
│       ├── windows/              # 14 files (~14 MB): adb.exe, fastboot.exe, mke2fs...
│       └── linux/                # 9 files (~16 MB): adb, fastboot, mke2fs...
│
├── memory-bank/                  # Project memory — 6 core files, source of truth
├── .agents/skills/               # Agent skills (read SKILL.md before coding)
├── docs/                         # Preserved legacy Go/Wails reference archive
├── CLAUDE.md                     # This file — project rules and patterns
└── package.json                  # pnpm scripts, deps, build pipeline
```
...
| :----------------------------------- | :---------------------------------------------------------- | :-------------------------------------------------- |
| **integrating-tauri-rust-frontends** | Tauri v2 frontend integration, IPC, event system            | `.agents/skills/integrating-tauri-rust-frontends/`  |
| **rust-refactor-helper**             | Refactoring Rust code, modularization, code smells           | `.agents/skills/rust-refactor-helper/`               |
| **m06-error-handling**              | Rust error handling patterns (Result, anyhow, thiserror)     | `.agents/skills/m06-error-handling/`                 |
| **m01-ownership**                   | Rust ownership, borrowing, lifetimes                        | `.agents/skills/m01-ownership/`                      |
| **m03-mutability**                  | Rust mutability, interior mutability, Mutex, RefCell         | `.agents/skills/m03-mutability/`                     |
| **m07-concurrency**                 | Rust concurrency (threads, async, channels, atomics)         | `.agents/skills/m07-concurrency/`                    |
| **unsafe-checker**                  | Auditing unsafe blocks in Rust code                          | `.agents/skills/unsafe-checker/`                     |
| **rust-code-navigator**             | Navigating and understanding Rust codebases                  | `.agents/skills/rust-code-navigator/`                |

### React & Frontend

| Skill                                | When to Use                                                 | Path                                                |
| :----------------------------------- | :---------------------------------------------------------- | :-------------------------------------------------- |
| **shadcn**                           | shadcn/ui component management, styling, composition        | `.agents/skills/shadcn/`                             |
| **vercel-react-best-practices**      | React patterns, hooks, performance                          | `.agents/skills/vercel-react-best-practices/`        |
| **vercel-composition-patterns**      | React component composition, state patterns                  | `.agents/skills/vercel-composition-patterns/`        |
| **web-design-guidelines**            | UI/UX design principles                                     | `.agents/skills/web-design-guidelines/`              |
